        ...
        <div className="p-10 h-full flex flex-col animate-fade-in text-ink max-w-[1600px] mx-auto w-full">
            <div className="flex justify-between items-end mb-10">
                <div>
                    ...
                    <p className="text-stone-500">Moderate discussions and community interactions.</p>
                </div>
                ...
                <div className="overflow-x-auto custom-scrollbar flex-1">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-stone-50 border-b border-stone-100 sticky top-0 z-10">
                            <tr>
                                <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-stone-400 w-1/4">Author</th>
                                <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-stone-400 w-1/2">Comment</th>
                                <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-stone-400">Context</th>
                        ...
                        <tbody className="divide-y divide-stone-50">
                            {filteredComments.length > 0 ? (
                                filteredComments.map((comment) => (
                                    <tr key={comment.id} className="hover:bg-stone-50/50 transition-colors group">
                                        <td className="py-4 px-6 align-top">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-stone-100 border border-stone-200 overflow-hidden shrink-0">
                                                    {comment.user?.avatar ? (
                                                        <img src={comment.user.avatar} alt={comment.user.username} className="w-full h-full object-cover" />
                                                    ) : (
                                                        <div className="w-full h-full flex items-center justify-center text-stone-400">
                                                            <div className="w-4 h-4 bg-stone-300 rounded-full" />
                                                        </div>
                                                    )}
                                                </div>
                                                <div>
                                                    <div className="font-bold text-ink text-sm">{comment.user?.username || 'Unknown'}</div>
                                                    <div className="text-[10px] text-stone-400">{formatDate(comment.createdAt)}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6 align-top">
                                        ...
                                        <td className="py-4 px-6 align-top">
                                            {comment.postTitle ? (
                                                <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-stone-100 text-stone-500 text-xs truncate max-w-[150px]">
                                                    <IconMessageSquare className="w-3 h-3 shrink-0" />
                                                    <span className="truncate">{comment.postTitle}</span>
                                                </span>
                                            ) : (
                                                <span className="text-xs text-stone-300 italic">Unknown Post</span>
                                            )}
                                        </td>
