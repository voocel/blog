...
const Header: React.FC<HeaderProps> = ({ onAdminClick, onAboutClick, onHomeClick, onSettingsClick }) => {
      ...
      <header className={`fixed top-0 left-0 w-full z-40 transition-all duration-500 ${scrolled
        ...
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <button onClick={onHomeClick} className="text-xs uppercase tracking-[0.2em] font-medium text-stone-600 hover:text-ink transition-colors cursor-pointer">Journal</button>
            ...
            <button onClick={onAboutClick} className="text-xs uppercase tracking-[0.2em] font-medium text-stone-600 hover:text-ink transition-colors cursor-pointer">About</button>
          </nav>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-paper z-30 flex flex-col items-center justify-center animate-fade-in md:hidden">
          <nav className="flex flex-col gap-8 text-center w-full px-8">
            <button onClick={() => { onHomeClick(); setMobileMenuOpen(false); }} className="text-3xl font-serif text-ink cursor-pointer">Journal</button>
            <button onClick={() => setMobileMenuOpen(false)} className="text-3xl font-serif text-stone-500 cursor-pointer">Collections</button>
            <button onClick={() => { onAboutClick(); setMobileMenuOpen(false); }} className="text-3xl font-serif text-stone-500 cursor-pointer">About</button>
