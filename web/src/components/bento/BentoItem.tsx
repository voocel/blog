...
const BentoItem: React.FC<BentoItemProps> = ({ children, className = "", title }) => {
    return (
        <div className={`bg-white/40 backdrop-blur-xl border border-white/50 shadow-sm rounded-3xl p-6 transition-transform duration-300 hover:scale-[1.01] hover:shadow-md nav-no-drag ${className}`}>
            {title && (
