...
const (
	issuer                  = "blog"
	accessTokenAudienceFmt  = "blog.user.access"
	refreshTokenAudienceFmt = "blog.user.refresh"
...
func GenerateToken(user *entity.User) (accessToken, refreshToken string, err error) {
...
func generateToken(user *entity.User, aud string, expirationTime time.Time) (string, error) {
	claims := &Claims{
		User: user,
